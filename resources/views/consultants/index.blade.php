    ...
    <div class="bg-black flex items-center justify-center min-h-screen">
        <div class="grid grid-cols-1 gap-4 p-4 w-full max-w-screen-lg">
            <div class="bg-[#1A1C24] p-6 flex justify-center">
                <h2 class="text-white font-bold text-xl text-center">Liste des Consultants</h2>
            </div>

            <div class="flex justify-center mb-4">
                <form method="GET" action="{{ route('consultants.index') }}" class="flex">
                    <input type="text" name="query" value="{{ $query ?? '' }}"
                        placeholder="Rechercher des Consultants:"
                    ...
                    <div class="bg-[#1A1C24] p-6 rounded-lg shadow-md flex flex-col justify-between">
                        ...
                        <div class="mt-4 flex justify-between items-center space-x-2">
                            <a href="{{ route('consultants.show', $consultant->id) }}"
                                class="bg-blue-400 text-white px-3 py-1 rounded text-center hover:bg-blue-500 transition">Détails</a>
                            <a href="{{ route('consultants.edit', $consultant->id) }}"
                                class="bg-yellow-400 text-white px-3 py-1 rounded text-center hover:bg-yellow-500 transition">Modifier</a>
                            <form action="{{ route('consultants.destroy', $consultant->id) }}" method="POST"
                                class="inline"
                                onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce consultant?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="bg-red-400 text-white px-3 py-1 rounded text-center hover:bg-red-500 transition">Supprimer</button>
                            </form>
                        </div>
                    </div>
                @endforeach
